...
`SystemDefinition -> Catalog/Assembly -> Validation -> Compiler -> ReleaseArtifact -> verified ArtifactPayload -> PublishedRelease -> Deploy + external auth binding -> autonomous generated Runtime -> authenticate declared active identity -> establish valid session -> actor context -> representative authenticated Runtime request`.

The same proof must exercise invalid authentication, unknown/disabled identity, invalid/expired session and missing authentication binding. Builder and Observe must be unavailable during ordinary Runtime behavior.

# Acceptance criteria
- all predecessor modules are invoked through their actual executable APIs where available;
- one representative identity authenticates and establishes a valid session;
- the session reaches actor context on an existing Runtime operation;

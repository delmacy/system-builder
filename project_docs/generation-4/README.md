...
The Shared Semantic Kernel / Capability Exchange Plane hypothesis explores whether replaceable suite capabilities can share a very small structural language and explicit interaction contracts while remaining independently owned and deployable. The logical plane is not a requirement for a central broker, shared database or ESB. Contract compatibility is a guarantee vector rather than a schema/version boolean. Long-lived occurrences additionally require per-obligation contract lineage. Security support is an independent horizon. Autonomous runtime security requires locally durable trust continuity, bounded security-policy freshness and explicit reconciliation after disconnection. Late security reconciliation should retain only purpose-qualified witness material. Federated evidence exchange needs purpose-bounded correlation. Privacy-preserving resource governance must distinguish issuance conservation from redemption privacy. Issuer accountability separates preventive conservation from retrospective detection. Witness governance makes quorum security conditional on declared correlated-failure assumptions. Multi-domain evidence composition keeps independently valid evidence as a qualified vector. Composition-policy lifecycle adds anti-rollback, semantic non-downgrade and bounded overlap. Semantic policy-diff proof requires decision/guarantee implication over an explicit proof scope. Proof-carrying policy research adds the consumer verification boundary: a runtime may verify a qualified derivation locally, but only when proposition/dependency binding, translation assurance, proof calculus/rules, trusted steps, verifier TCB/currentness and resource bounds are explicit. Verifier trust-continuity research adds that checker generations themselves require anti-rollback/currentness and explicit qualification against immutable normative proof-semantics profiles; differential checking, formal verification, conformance corpora, reproducible builds and diverse builds are complementary evidence classes rather than interchangeable meanings of `verified`. Normative proof-semantics governance adds immutable semantic snapshots, explicit errata/correction/supersession relations, separate language/rule/theory/extension identity, and the rule that current security admissibility may supersede an old interpretation without rewriting historical proof meaning. Downgrade-resistant profile negotiation adds that support, admissibility and preference are distinct sets; selection is bound to immutable profile identity, peer/trust context and security floors; rolling upgrade may use bounded relationship-specific skew without allowing weaker compatibility support to become an attacker-controlled fallback. Negotiation-evidence lifecycle research adds explicit pinning scope, separate admission/continuation/new-effect decisions, queued-work and stream-generation lineage, partition reconciliation without historical rewriting, and the separation of implementation rollout/rollback from semantic activation and security-floor monotonicity. Semantic-generation handoff research adds partial-order generation frontiers, branch-local versus invariant-scope cutover, mixed-generation join qualification, and the requirement to fence conflicting old-generation effect rights rather than infer safety from queue drain, stream completion or marker observation. Handoff-recovery/compaction research adds proof-preserving frontier summaries, explicit below-compaction-floor semantics, independent replay/dedup/resolvability/security horizons, and durable negative/fencing evidence sufficient to prevent stale-work resurrection after detailed transport history is discarded.

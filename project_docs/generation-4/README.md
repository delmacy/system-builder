...
1. **Product UX, Living Canvas & AI-Native Builder** — `research/G4_PRODUCT_UX_AI_NATIVE_BUILDER.md`; frontend/design-system foundation plan — `research/G4_FRONTEND_DESIGN_SYSTEM_FOUNDATION_PLAN.md`; interaction-state research — `research/G4_UI_INTERACTION_STATE_MATRIX_RESEARCH.md`; main semantic 3D composition/deployment research — `research/G4_MAIN_COMPOSITION_CANVAS_3D_RESEARCH.md`
...
8. **Shared Semantic Kernel / Capability Exchange Plane / Inter-Capability Boundary Model**
   ...
   - `research/G4_CAPABILITY_EXCHANGE_DELEGATION_SEMANTIC_MAPPING_PROOF.md`
...
- `Protocol/credential translation != semantic authority translation proof`; heterogeneous permission models require directional effective-authority containment over an explicit effect/resource/context scope.
- `Role/scope name equality != authorization semantic equivalence`; deny, conditions, hierarchy, session/resource policy and meta-capability semantics remain material.
- `Target re-authorization != attenuation`; independently granted target authority creates a new authority edge and revocation/currentness domain.
- `Mapping unchanged != mapping proof current`; provider/API permission-universe growth can defeat a prior attenuation proof.

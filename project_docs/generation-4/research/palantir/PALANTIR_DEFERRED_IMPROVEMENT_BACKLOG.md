...
| IMP-PAL-037 | AI tool invocation must resolve an effective execution principal and re-authorize independently of model tool selection | AIP Logic model-requested tools executed under invoking-user permissions | Prevents inference/prompt injection from becoming execution authority | HIGH | AI/tool execution architecture |
| IMP-PAL-038 | Bind human/session approval to concrete effect proposal, scope and revalidation conditions | Chatbot Action/Command confirmation + AI FDE mutating-tool approval | Prevents approval from silently authorizing changed payload/tool/context | HIGH | AI/action admission design |
| IMP-PAL-039 | Preserve argument provenance and snapshot/currentness for deterministic AI tool inputs | Chatbot deterministic Action/Function inputs pinned at reasoning-loop start | Prevents deterministic-but-stale arguments from being presented as current/authorized | MEDIUM | AI/tool contract and Station review design |
| IMP-PAL-040 | Treat AI/MCP/provider egress as an explicit governance-domain transition with destination/provider policy requalification | Palantir MCP local/external data-governance boundary | Prevents authorized source reads from silently becoming unrestricted disclosure/retention at external AI providers | HIGH | AI provider/data-governance architecture |
| IMP-PAL-041 | Give every agent/application tool surface an explicit maximum resource/operation authority envelope independent of caller/service principal breadth | Ontology MCP OAuth + Developer Console application restrictions | Prevents a broadly privileged user/service account from making every capability reachable through one generated application or agent | HIGH | Capability closure / application authorization design |
| IMP-PAL-042 | Preserve explicit principal-transition/delegation lineage across nested agent/function/action/automation calls | Pro-code agents published as Functions + Automate owner/service-user execution + Function-backed Action semantics | Prevents caller identity from being silently treated as transitive authority and exposes service/owner amplification | HIGH | AI/workflow delegation architecture |
| IMP-PAL-043 | Version and requalify multidimensional application/function capability closure, including elevated execution gates | Function registry/resource permissions + Developer Console restrictions + extended-capability allowlists | Prevents callable/discoverable code from silently acquiring stale or excessive data/effect authority as dependencies evolve | HIGH | Capability closure / application authorization design |

## Immediate visual-work implications

Only representational constraints directly relevant to Station may be consumed now. Existing requirements remain: currentness/evidence distinct from effective truth; connectivity distinct from health; AI suggestion distinct from authorization/effect; semantic identity distinct from display/API names; relayed/imported state distinct from live/direct authority; version freshness distinct from readiness; imported bundles represented through transfer/admission/application states; signature validity distinct from currentness/approval; preview effects visibly suppressed/attempted/observed; execution authority distinct from telemetry-inspection authority; log/trace absence qualified by freshness/retention.

Future AI/action review surfaces should distinguish `MODEL_PROPOSED`, `TOOL_SELECTED`, `EXECUTION_PRINCIPAL_RESOLVED`, `APPROVAL_REQUIRED/APPROVED`, `EXECUTION_ATTEMPTED`, and `EFFECT_OBSERVED/UNKNOWN`; consequential arguments should expose whether they are model-generated, user-provided, application-derived or revision/snapshot-pinned. External-provider/MCP use should be representable as an egress/policy-domain transition, and tool discoverability should not visually imply invocation authority. Station collecting an approval must not be represented as Station itself possessing execution authority.

This round adds a representational constraint only: when nested execution is eventually surfaced, `INITIATOR` and `EFFECTIVE_PRINCIPAL` must be representable as different identities, and `CALLABLE/DISCOVERABLE` must remain visually distinct from `AUTHORIZED/ADMISSIBLE`. This does not authorize backend or UI implementation now.

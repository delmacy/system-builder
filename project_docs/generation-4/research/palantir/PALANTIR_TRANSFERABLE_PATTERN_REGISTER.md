...
| PT-45 | Pro-code agent as Function + Automate owner/service-user execution + Function-backed Action special authorization | initiator identity != necessarily effective downstream principal | `ADOPT PRINCIPLE` | `DEFERRED_IMPROVEMENT` | nested calls must record/re-authorize principal transitions; owner transfer/service identity can amplify authority relative to initiator |
| PT-46 | Function repository/registry/resource permissions + Developer Console restrictions + extended-capability allowlists | callable artifact != data authority != elevated-effect authority | `ADAPT` | `DEFERRED_IMPROVEMENT` | function/version/resource closure and elevated policy drift independently; discovery/callability cannot imply admissibility |
| PT-47 | Function read-time row/column filtering does not automatically protect outputs | authorized input view != automatically authorized derived output | `ALREADY COVERED` | `DEFERRED_IMPROVEMENT` | principal lineage and classification lineage are independent; derived output requires explicit downstream protection |
...
23. How should nested `agent -> function/action/automation -> agent/provider` calls preserve initiator, effective principal, authority source, delegation lineage and re-authorization evidence without assuming transitive caller identity?
24. Which changes to owner/service principal, function version, resource closure, application restrictions or elevated-capability policy invalidate an earlier authority qualification?

## Saturation

Not saturated. Nested principal propagation now has concrete primary evidence showing multiple principal-selection semantics rather than one transitive identity model. Repeated searches for basic MCP/tool authorization have low marginal value. Next pass should prioritize application/tool closure versioning and requalification when Ontology/capabilities change, then delegated-principal currentness/ownership transfer and external telemetry policy propagation. Bundle verification/replay remains open if stronger primary evidence appears.
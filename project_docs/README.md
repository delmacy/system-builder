...
- Sprint — unidade operacional/review que contém trabalho executável bounded.
...
Product development uses Sprint Mode:

- `schedule/SPRINT_MODE.md` — branch/review/autonomy contract;
- `schedule/SPRINT_REPORT_TEMPLATE.md` — compact Sprint review evidence;
- `execution_planning/PRODUCT_10_SPRINT_PLAN.md` — first committed ten-Sprint product horizon.

The first product horizon starts after the completed ProcessMirror contract and progresses Recipe → Analysis → Design → downstream contract spine → Catalog → Assembly → Validation → Compiler → Release → Deploy, maximizing a testable vertical slice before Observe/Support expansion.
...
8. Uma Sprint deve terminar com validação observável e relatório curto antes de integração em `main`.

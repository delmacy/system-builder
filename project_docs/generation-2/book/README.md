...
A pesquisa adversarial continua `ACTIVE / NOT_SATURATED`; no snapshot editorial revalidado em 2026-09-06, Full Passes 1–7 estão completos e o Full Pass 8 está em 9/28 capabilities e 9/12 mandatory clusters, com 7/8 full passes mínimos completos e 408 findings materiais (284 edge scenarios + 124 ConflictPatterns). O avanço desde o snapshot anterior permanece elegível como `NO-NEW-MATERIAL`: os revisits mais recentes reconfirmam separações já explicadas, como `deployment success != runtime-effective != business convergence`, `desired replicas != actual resource pressure`, `telemetry absence != runtime absence/health` e `rollback de topologia != reversão/quiescência de efeitos`. Isso reforça a estabilidade do entendimento já publicado, mas **não equivale ainda a saturação**. Elicitation & System Understanding e Operability Elicitation permanecem lentes transversais em pesquisa. A negative-space final ainda não começou e Planning C continua bloqueado. Por isso, capítulos publicados devem permanecer estáveis enquanto novos resultados apenas reconfirmarem padrões existentes; revisão de versão continua reservada a mudança material de compreensão.

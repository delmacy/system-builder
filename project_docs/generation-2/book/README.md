...
A pesquisa adversarial continua `ACTIVE / NOT_SATURATED`; no snapshot editorial revalidado em 2026-09-06, Full Passes 1–7 estão completos e o Full Pass 8 está em 20/28 capabilities e 12/12 mandatory clusters, com 7/8 full passes mínimos completos e 408 findings materiais (284 edge scenarios + 124 ConflictPatterns). O revisit mais recente, Enterprise Trust / PKI / Certificate Lifecycle, foi `ELIGIBLE NO-NEW-MATERIAL`: trust-epoch crossing, restore-cut differential, validade versus autoridade corrente, revocation split-view, cohorts offline com anchors antigos, provider substitution, pressão de filas e as lentes de Elicitation/Operability foram absorvidos pelas famílias já existentes. Isso reforça separações já ensinadas — validade criptográfica não é autoridade corrente, recuperação técnica não é requalificação automática de confiança e evidência de assinatura não deve ser fortalecida além do que efetivamente prova —, mas **não constitui entendimento materialmente novo e não equivale ainda a saturação**. Elicitation & System Understanding e Operability Elicitation permanecem lentes transversais em pesquisa. A negative-space final ainda não começou e Planning C continua bloqueado. Por isso, capítulos publicados devem permanecer estáveis enquanto novos resultados apenas reconfirmarem padrões existentes; revisão de versão continua reservada a mudança material de compreensão.

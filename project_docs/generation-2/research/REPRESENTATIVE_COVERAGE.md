...
VS Code Marketplace/runtime security/Workspace Trust/web extension sandbox: `DEEP`; Terraform provider requirements/lock/install mirrors: `DEEP`; OSGi Core 8 BundleRevision/BundleWiring/lifecycle: `DEEP`; Backstage frontend/backend plugin and extension-point architecture: `DEEP`; Kubernetes dynamic + manifest-based admission: `DEEP`; USENIX malicious-plugin/npm/native-extension ecosystem studies: `DEEP`. Findings `G2-FINDING-EXT-30..37`; material new findings; consecutive-no-material = 0; NOT SATURATED. Focus: integrity/trust/admission/containment/authority separation; revision-bound dependency closure; ambiguous install/revoke/remove outcomes; residual-in-use revocation evidence; containment requalification; control-path failure/bootstrap governance; faceted non-amplifying Station administration; AI/AGWS extension boundary; qualified local closure.

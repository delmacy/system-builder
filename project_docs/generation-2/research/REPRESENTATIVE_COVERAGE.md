...
### Observability / Operations / Incident — revisit 5
OpenTelemetry Sampling + Collector: `DEEP`; OpenTelemetry semantic conventions/specification: `DEEP`; Prometheus Remote Write + OTLP ingestion ordering/staleness: `DEEP`; Grafana Alerting No Data/Error/state health: `DEEP`; PagerDuty incidents/escalation lifecycle: `DEEP`; Google SRE SLO alerting: `DEEP`. Findings `G2-FINDING-OOI-39..46`; material new findings; consecutive-no-material = 0; NOT SATURATED. Focus: typed operational evidence lifecycle, multi-axis instrumentation/sampling/query/policy revisions, explicit NoData/Error/Stale/PARTIAL/INCONCLUSIVE semantics, sampling as selection contract, partial-order chronology, alert-notification-incident separation, acknowledgement ownership lineage, expected-observation population denominator, provider portability, qualified local closure and non-amplifying Station/AI/AGWS authority.

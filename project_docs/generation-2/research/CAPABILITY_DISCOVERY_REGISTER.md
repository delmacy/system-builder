...
| G2-CAPABILITY-CANDIDATE-RENDERER-BINDING-DECISION-EVIDENCE | CROSS_CUTTING | JSON Forms renderer registry/testers + prior provider/binding findings | CANDIDATE | Promote if generated-experience planning and provider/binding reconciliation require one reusable renderer-selection/binding evidence envelope. |
| G2-CAPABILITY-CANDIDATE-DESIGN-TOKEN-REFERENCE-LINEAGE | DOMAIN | DTCG portable design-token exchange + UI design-system lifecycle | CANDIDATE | Promote only if repository archaeology shows portable design decisions need explicit token/reference lifecycle beyond ordinary presentation metadata. |
| G2-CAPABILITY-CANDIDATE-GENERATED-EXPERIENCE-CONFORMANCE-CASE | CROSS_CUTTING | Storybook portable states + render/interaction/accessibility/visual testing | CANDIDATE | Promote if named state/proof cases recur as a reusable acceptance primitive across UI, accessibility, release and observability. |
| G2-CAPABILITY-CANDIDATE-EFFECT-GUARANTEE-QUALIFICATION | CROSS_CUTTING | DBOS transaction/step guarantees + durable workflow representatives | CANDIDATE | Promote if Integration/Automation and provider reconciliation require one reusable scoped delivery/atomicity/idempotency claim. |
| G2-CAPABILITY-CANDIDATE-RECOVERY-ATTEMPT-LINEAGE | CROSS_CUTTING | Step Functions redrive + distributed durable recovery | CANDIDATE | Promote if failure-recovery/provenance synthesis confirms shared recovery-attempt lineage semantics. |
| G2-CAPABILITY-CANDIDATE-SEMANTIC-MIGRATION-PROOF | CROSS_CUTTING | Camunda migration constraints + lifecycle migration research | CANDIDATE | Promote if lifecycle/data/schema migration domains converge on post-migration semantic proof obligations. |

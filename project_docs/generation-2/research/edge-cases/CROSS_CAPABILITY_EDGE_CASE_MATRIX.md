...
| --- | --- | --- | --- | --- |
| 3 | Identity × Authorization × Station × AGWS × AI | COVERED / NO NEW MATERIAL CLASS | `ADAPTIVE_GOVERNED_WORK_SURFACES_FULL_PASS_2_REVISIT.md` | duplicate-screened; eligible no-material streak **1** |
...
| 2 | Workflow × Integration × Messaging × external mutation | COVERED / MATERIAL NEW CLASS | `WORKFLOW_DURABLE_EXECUTION_FULL_PASS_2_REVISIT.md` | `G2-XEDGE-WORKFLOW-INTEGRATION-MSG-005`; streak **0** |
| 4 | Data/Schema × Privacy × Storage × Lifecycle | COVERED / MATERIAL NEW CLASS | `DATA_SCHEMA_MIGRATIONS_FULL_PASS_2_REVISIT.md` | `G2-XEDGE-DATA-PRIVACY-STORAGE-LIFECYCLE-005`; streak **0** |
| 6 | Provider/Binding × external realizations | COVERED / NO NEW MATERIAL CLASS | `STORAGE_DOCUMENTS_MEDIA_FULL_PASS_2_REVISIT.md` | duplicate-screened; eligible no-material streak **1** |
...
| 5 | Build × Artifact/Release × Deployment × Runtime | COVERED / MATERIAL NEW SCENARIO | `BUILD_DEPENDENCY_GRAPH_REPRODUCIBILITY_FULL_PASS_2_REVISIT.md` | `G2-XEDGE-BUILD-RELEASE-DEPLOY-RUNTIME-005`: byte-reproducible output can still have incompatible producing provenance/authority/policy qualification; no new reusable ConflictPattern after duplicate screening; streak **0** |

Build Pass-2 also adds `G2-EDGE-BUILD-007..008`: per-dependency validity may not establish one coherent qualified resolution cut, and target-conditioned effective closure can change without a canonical build-intent revision. These map to existing qualification-join, revision-vector, currentness, effective-identity, provider-qualification, attestation-qualification, trust-authority and qualified-claim patterns.

## Full Pass 2 campaign state

- canonical capabilities revisited: **7/28**;
- mandatory clusters revisited: **7/12**;
- Identity × Authorization × Station × AGWS × AI streak: **1**;
- Provider/Binding × external realizations streak: **1**;
- Build × Artifact/Release × Deployment × Runtime streak: **0**;
- other material-revisit cluster streaks: **0**;
- material edge scenarios: **257**;

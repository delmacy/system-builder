...
function validateSchema(value: unknown, schema: JsonSchema, path = "$"): readonly string[] {
  ...
  if (schema.type === "object") {
    ...
    if (schema.additionalProperties === false) {
      for (const key of Object.keys(value)) if (!(key in properties)) errors.push(`${path}.${key}: additional property`);
    }
...
test("SystemDefinition accepts explicit bounded action effects and workflow transitions", () => {
  ...
  definition.processes = [{ id: "process:ticket", name: "Ticket lifecycle", requirementRefs: ["REQ-2"], states: ["open", "closed"], transitions: [{ id: "transition:close", from: "open", to: "closed", actionRef: "action:close-ticket" }] }];
...
test("executable semantics reject malformed, ambiguous and value-bearing declarations", () => {
  ...
  leaking.processes = [{ id: "process:ticket", name: "Ticket lifecycle", requirementRefs: ["REQ-2"], states: ["open", "closed"], transitions: [{ id: "transition:close", from: "open", to: "closed", actionRef: "action:close-ticket", value: "secret" }] }];

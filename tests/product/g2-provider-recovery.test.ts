...
test("provider recovery permits retry only after explicit authoritative reconciliation of current authoritative evidence",()=>{
  const current=qualification();
  assert.throws(()=>normalizeProviderRecovery({contractVersion:"1.0.0",previous:current,current,reconciliationOutcome:"NOT_RECONCILED",disposition:"RETRY_ELIGIBLE"}),/explicit authoritative reconciliation/);
  assert.equal(normalizeProviderRecovery({contractVersion:"1.0.0",previous:current,current,reconciliationOutcome:"AUTHORITATIVELY_RECONCILED",disposition:"RETRY_ELIGIBLE"}).disposition,"RETRY_ELIGIBLE");
});
...
test("conflicting evidence remains reconciliation-required even when current evidence looks supported",()=>{
  const current=qualification();
  assert.equal(normalizeProviderRecovery({contractVersion:"1.0.0",previous:current,current,reconciliationOutcome:"CONFLICTING",disposition:"RECONCILE_REQUIRED"}).disposition,"RECONCILE_REQUIRED");
  assert.throws(()=>normalizeProviderRecovery({contractVersion:"1.0.0",previous:current,current,reconciliationOutcome:"CONFLICTING",disposition:"RETRY_ELIGIBLE"}),/explicit authoritative reconciliation|conflicting evidence/);
});

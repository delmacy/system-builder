...
import {
  assertIncidentAuthority, assertSloTargetsSliRevision, canAggregatePopulationCoverage, canPromoteConditionToAlert,
  isObservationComplete, isReconciliationEvidenceComplete, reconciliationDisposition, reconciliationRequiresRetryGuard,
  requireReconciliationBeforeRetry,
  type EvaluatedCondition, type ObservabilityEvidence, type ObservabilityIncident, type ReconciliationEvidence,
  type ReconciliationJob, type ServiceLevelIndicatorDefinition, type ServiceLevelObjectiveTarget, type SliObservation, type TelemetrySignal,
} from "../../packages/contracts/observability/index.js";

const knownEvidence: ObservabilityEvidence = { source: { sourceId: "telemetry:cpu", producerId: "provider:otel", producerRevision: "1.4.2", locality: "STATION", populationRef: "station:alpha" }, observedAt: "2026-09-16T07:30:00Z", effectiveAt: "2026-09-16T07:29:58Z", currentness: "CURRENT", evidenceState: "KNOWN" };
...
test("population aggregation cannot manufacture completeness", () => {
  ...
  assert.equal(canAggregatePopulationCoverage([{ populationRef: "station:alpha", locality: "STATION", currentness: "CURRENT", evidenceState: "KNOWN" }, { populationRef: "station:beta", locality: "STATION", currentness: "UNKNOWN", evidenceState: "PARTIAL" }]), false); assert.equal(canAggregatePopulationCoverage([]), false);
});

const reconciliationJob: ReconciliationJob = { reconciliationId: "reconcile:17", sourceRevision: "source@4", effectRevision: "effect@9", locality: "FLEET", intendedPopulation: ["station:alpha", "station:beta"] };
const completeReconciliation: ReconciliationEvidence = { ...reconciliationJob, attemptedPopulation: ["station:alpha", "station:beta"], observedPopulation: ["station:alpha", "station:beta"], currentness: "CURRENT", evidenceState: "KNOWN", disposition: "CONVERGED" };

test("reconciliation proves exact population locality revisions and currentness before convergence", () => {
  assert.equal(isReconciliationEvidenceComplete(reconciliationJob, completeReconciliation), true);
  assert.equal(reconciliationDisposition(reconciliationJob, completeReconciliation), "CONVERGED");
  assert.equal(reconciliationRequiresRetryGuard(reconciliationJob, completeReconciliation), false);
});

test("partial populations cannot imply global convergence", () => {
  const partial = { ...completeReconciliation, observedPopulation: ["station:alpha"], evidenceState: "PARTIAL" as const };
  assert.equal(isReconciliationEvidenceComplete(reconciliationJob, partial), false); assert.equal(reconciliationDisposition(reconciliationJob, partial), "UNKNOWN"); assert.equal(reconciliationRequiresRetryGuard(reconciliationJob, partial), true);
});

test("stale evidence locality mismatch revision mismatch and UNKNOWN require reconcile-before-retry", () => {
  for (const evidence of [
    { ...completeReconciliation, currentness: "STALE" as const },
    { ...completeReconciliation, locality: "STATION" as const },
    { ...completeReconciliation, effectRevision: "effect@8" },
    { ...completeReconciliation, disposition: "UNKNOWN" as const },
  ]) {
    assert.equal(reconciliationDisposition(reconciliationJob, evidence), "UNKNOWN"); assert.equal(reconciliationRequiresRetryGuard(reconciliationJob, evidence), true);
  }
});

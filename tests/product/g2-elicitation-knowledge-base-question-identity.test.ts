...
function occurrence(definition = definitionRevision("r1", "Which assets are critical?")) {
  return normalizeQuestionOccurrence({
    contractVersion: ELICITATION_KNOWLEDGE_BASE_CONTRACT_VERSION,
    ref: {
      contractVersion: SEMANTIC_SUBSTRATE_CONTRACT_VERSION,
      semanticOwner: owner,
...
test("question definition revision and occurrence have distinct immutable identities", () => {
  ...
  assert.equal(asked.ref.occurrenceRef, "occurrence:workshop-2026-09-08:asset-criticality");
...
test("owner, kind, canonical identity and required context fail closed", () => {
  ...
  const base = {
    ...
    definitionRevision: definition.ref,
    context: { contextRef: "workshop:1", populationScope: "ops", localityScope: "station:a" },
  };

  assert.throws(() => normalizeQuestionOccurrence({ ...base, ref: { ...base.ref, semanticOwner: "capability:other" } }), /owner must match/);
  assert.throws(() => normalizeQuestionOccurrence({ ...base, ref: { ...base.ref, semanticKind: "domain-truth" } }), /semanticKind/);
  assert.throws(() => normalizeQuestionOccurrence({ ...base, ref: { ...base.ref, canonicalRef: "question:other" } }), /canonical identity must match/);
...
test("equal labels cannot collapse owner-qualified revision and occurrence coordinates", () => {
  ...
  assert.notEqual(definition.ref.definitionRef, asked.ref.occurrenceRef);
});

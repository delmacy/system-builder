...
test("permission evaluation allows only an exact declared role resource and action", () => {
  ...
  const allowed = evaluateRuntimePermission({
    ...
    permissions: [permission("role:operator", "entity:ticket", ["action:read", "action:update"])],
...
test("permission evaluation defaults to deny with no wildcard or permissive fallback", () => {
  ...
  for (const [resourceRef, actionRef] of [
    ["entity:other", "action:read"],
...
test("permission context requires exact bounded membership and organization references", () => {
  const resolved = resolveRuntimeActorAuthority(
    model({ roleBindings: [{ id: "binding:ops", roleRef: "role:operator", membershipRef: "membership:ops" }] }),
    { identityRef: "identity:alice", memberships: [{ id: "membership:ops", active: true }] },
  );
  assert.equal(resolved.ok, true);
  if (!resolved.ok) return;
  ...
  const permissions = [{
    ...
    context: { membershipRef: "membership:ops", organizationRef: "organization:acme" },
  }];
  const denied = evaluateRuntimePermission({
    authority: resolved.authority,
    permissions,
    ...
    context: { membershipRef: "membership:ops", organizationRef: "organization:other" },
  });
  assert.equal(denied.allowed, false);
...
test("permissions with policy references fail closed until bounded policy evaluation is applied", () => {
  ...
  const decision = evaluateRuntimePermission({
    authority: resolved.authority,
    permissions: [{ ...permission("role:operator"), policyRefs: ["policy:office-hours"] }],
    resourceRef: "entity:ticket",
    actionRef: "action:read",
  });
...
test("permission evidence is deterministic and contains references and reasons only", () => {
  const authority = {
    kind: "RuntimeResolvedAuthority" as const,
    identityRef: "identity:alice",
    membershipRefs: [],
    roleRefs: ["role:reviewer", "role:operator"],
    roleBindingRefs: ["binding:reviewer", "binding:operator"],
  };
  const first = evaluateRuntimePermission({
    authority,
    permissions: [permission("role:reviewer"), permission("role:operator")],
    resourceRef: "entity:ticket",
    actionRef: "action:read",
  });
  const second = evaluateRuntimePermission({
    authority,
    permissions: [permission("role:operator"), permission("role:reviewer")],
    resourceRef: "entity:ticket",
    actionRef: "action:read",

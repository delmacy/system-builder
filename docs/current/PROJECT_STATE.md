...
## Current gate
Integrate this reconciliation only after exact-head gates pass, then execute TASK-526 from fresh main. TASK-526 is integrated Product Proof only and must not introduce new semantic ownership. Preserve one authoritative commit per TASK when required and blocker-first handling of CI/review/proof/memory drift.

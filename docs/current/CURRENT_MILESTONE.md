...
Fresh-main Planning & Materialization on base `56868c69898d884f9d5e55cdd0de984c97f429aa` materializes only `P19-RUNTIME-MATERIALIZATION-HANDOFF-01` / WBS 19.2.2 with TASK-439..443 in dependency order. Its goal is to connect the already integrated operator-bootstrap/factory result to the existing verified local-process Deploy path, launch the actual generated runtime in the already supported initial topology, and prove immutable release identity plus external EnvironmentProfile/secret boundaries without introducing a second deploy/runtime owner.

TASK-439 is first eligible after Planning integration. TASK-440..443 remain dependency-blocked. The Sprint must reuse existing Compiler/Release/Deploy/Runtime contracts and `runLocalProcessDeployment`; stale/substituted artifacts, hash/ref mismatch, unverifiable payload, incompatible runtime/environment, secret/migration/startup failures and leakage must remain fail-closed. Bootstrap progress/diagnostics are not deployment/runtime authority.

WBS 19.2.3+ remains forecast and non-executable. New deployment topology, persistent control plane, Builder-owned runtime dependency, new bounded context, Decision Boundary change or other L4 movement requires explicit ADR/change control rather than implicit materialization.

...
import { BUTTON_GROUP_DESCRIPTOR, ComponentRegistry, applyCompositionDraftMutation, createCompositionDraftTransaction, defineCompositionGraph, discardCompositionDraft, normalizeComponentDescriptor, projectCompositionPreview, validateCompositionGraph, validateCompositionPlacement, type CompositionDraftTransaction, type CompositionGraphFinding } from "../../../../packages/station-composition/index";
...
const labBaseGraph = defineCompositionGraph({ rootRef: "layer:button-group", nodes: [
  { ref: "layer:button-group", componentRef: BUTTON_GROUP_DESCRIPTOR.id },
  { ref: "layer:button-1", componentRef: labAtomicButton.id, placement: { parentRef: "layer:button-group", slotRef: "button-1", columnSpan: 2, rowSpan: 1 } },
  { ref: "layer:button-2", componentRef: labAtomicButton.id, placement: { parentRef: "layer:button-group", slotRef: "button-2", columnSpan: 2, rowSpan: 1 } },
] });
...
export function StationFoundationClient() {
  ...
  const [labDraft, setLabDraft] = useState<CompositionDraftTransaction>(() => createCompositionDraftTransaction(labBaseGraph, compositionRegistry));
  const [labRejectedFindings, setLabRejectedFindings] = useState<readonly CompositionGraphFinding[]>([]);
  ...
  const mutateLabDraft = () => { const node = labDraft.draft.nodes.find((candidate) => candidate.ref === "layer:button-1"); if (node === undefined) return; const result = applyCompositionDraftMutation(labDraft, { type: "replace-node", node: { ...node, placement: { parentRef: "layer:button-group", slotRef: "button-1", columnSpan: node.placement?.columnSpan === 3 ? 2 : 3, rowSpan: 1 } } }, compositionRegistry); if (result.ok) { setLabDraft(result.transaction); setLabRejectedFindings([]); } };
  const rejectInvalidLabDraft = () => { const result = applyCompositionDraftMutation(labDraft, { type: "replace-node", node: { ref: "layer:invalid", componentRef: "component:missing", placement: { parentRef: "layer:button-group", slotRef: "button-3", columnSpan: 2, rowSpan: 1 } } }, compositionRegistry); if (!result.ok) setLabRejectedFindings(result.findings); };
  const discardLabDraft = () => { setLabDraft((current) => discardCompositionDraft(current)); setLabRejectedFindings([]); };

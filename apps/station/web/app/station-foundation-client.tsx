...
export function StationFoundationClient() {
  ...
  const renderWindowBody = (definition: WindowDefinition) => {
    ...
    if (definition.id === "component-lab") return <div className="space-y-5 p-6"><div><h1 className="text-2xl font-semibold">Component Lab</h1><p className="mt-1 max-w-xl text-sm leading-6 text-muted-foreground">M1 inspection surface for source-owned primitives, semantic icons and presentation states.</p></div><div className="flex flex-wrap gap-2"><Button>Primary</Button><Button variant="outline">Outline</Button><Button variant="ghost">Ghost</Button><Badge>Badge</Badge><Badge>density:{presentation.appearance.density}</Badge><Badge>motion:{presentation.appearance.motion}</Badge><Badge>focus:{windows.activeWindowRef === null ? "none" : "window"}</Badge></div><div className="grid grid-cols-2 gap-2 sm:grid-cols-3">{ICON_TOKENS.map((token) => <div key={token} className="flex items-center gap-2 rounded-md border p-2 text-xs"><StationIcon token={token} label={token} /><code>{token}</code></div>)}</div></div>;

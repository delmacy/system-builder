...
function runtimeExecutionSupport(): string {
  return [
    ...
    "async function runtimeHandleActionRequest(request, response, model, connectionString) { const route = runtimeActionRoute(request.url); if (!route) return false; if (request.method !== \"POST\") { runtimeWriteJson(response, 405, { kind: \"RuntimeDiagnostic\", code: \"RUNTIME_ACTION_METHOD_NOT_ALLOWED\", detail: String(request.method || \"\") }); return true; } const action = model.actions.find((candidate) => candidate.id === route.actionId); if (!action) { runtimeWriteJson(response, 404, { kind: \"RuntimeDiagnostic\", code: \"RUNTIME_ACTION_UNKNOWN\", detail: route.actionId }); return true; } if (!action.effect) { runtimeWriteJson(response, 400, { kind: \"RuntimeDiagnostic\", code: \"RUNTIME_ACTION_UNSUPPORTED\", detail: route.actionId }); return true; } const entity = runtimeEntityById(model, action.effect.entityRef); if (!entity) { runtimeWriteJson(response, 400, { kind: \"RuntimeDiagnostic\", code: \"RUNTIME_ACTION_INVALID_TARGET\", detail: action.effect.entityRef }); return true; } try { let result; if (action.effect.kind === \"entity.create\") result = await runtimeEntityCreate(connectionString, entity, route.recordId, await runtimeReadJsonBody(request)); else if (action.effect.kind === \"entity.update\") result = await runtimeEntityUpdate(connectionString, entity, route.recordId, await runtimeReadJsonBody(request)); else if (action.effect.kind === \"entity.delete\") result = await runtimeEntityDelete(connectionString, entity, route.recordId); else { runtimeWriteJson(response, 400, { kind: \"RuntimeDiagnostic\", code: \"RUNTIME_ACTION_UNSUPPORTED\", detail: action.effect.kind }); return true; } if (!result.ok) { runtimeWriteJson(response, result.code === \"RUNTIME_ENTITY_NOT_FOUND\" ? 404 : 400, { kind: \"RuntimeDiagnostic\", code: result.code, detail: result.detail }); return true; } runtimeWriteJson(response, 200, { kind: \"RuntimeAction\", actionId: action.id, effect: action.effect.kind, record: result.value }); return true; } catch (error) { runtimeWriteJson(response, 503, { kind: \"RuntimeDiagnostic\", code: \"RUNTIME_ACTION_DATABASE_FAILED\", detail: error instanceof Error ? error.message : \"ACTION_DATABASE_FAILED\" }); return true; } }",
  ].join("\n");
}
...
function injectRuntimeExecution(entrypoint: string): string {
  ...
  if (!entrypoint.includes(supportMarker) || !entrypoint.includes(setupMarker) || !entrypoint.includes(routeMarker)) {

...
function injectRuntimeExecution(entrypoint: string): string {
  ...
  rendered = rendered.replace(setupMarker, [
    ...
    "            for (const provider of runtimeModel.authenticationProviders || []) { const value = process.env[provider.bindingRef]; if (typeof value !== \"string\" || value.length === 0) fail(\"RUNTIME_AUTH_BINDING_UNRESOLVED\", provider.bindingRef); }",
  ...
  rendered = rendered.replace(routeMarker, [
    "              if (runtimeModel && await runtimeHandleAuthenticationRequest(request, response, runtimeModel)) return;",

...
export type IconButtonProps = Omit<ButtonProps, "size" | "children"> & Readonly<{
  label: string;
  children: ReactNode;
}>;
...
export function Input({ className, type = "text", ...props }: InputHTMLAttributes<HTMLInputElement>) {
  return createElement("input", {
    ...
    "data-slot": "input",
    className: cn(
      "flex h-9 w-full min-w-0 rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-xs transition-colors placeholder:text-muted-foreground disabled:pointer-events-none disabled:opacity-50 focus-visible:border-ring focus-visible:ring-2 focus-visible:ring-ring/40",
      className,
    ),
  });
}
...
export function Select({ className, children, ...props }: SelectHTMLAttributes<HTMLSelectElement>) {
  return createElement(
    "select",
    {
      ...
      "data-slot": "select",
      className: cn(
        "flex h-9 w-full rounded-md border border-input bg-background px-3 py-1 text-sm shadow-xs transition-colors disabled:pointer-events-none disabled:opacity-50 focus-visible:border-ring focus-visible:ring-2 focus-visible:ring-ring/40",
        className,
      ),
    },
    children,
...
export type ToggleProps = Omit<ButtonHTMLAttributes<HTMLButtonElement>, "aria-pressed"> & Readonly<{
  pressed?: boolean;
...
export type SeparatorProps = HTMLAttributes<HTMLDivElement> & Readonly<{
...
export type TooltipProps = HTMLAttributes<HTMLSpanElement> & Readonly<{
  content: ReactNode;
}>;
...
export function Tooltip({ className, content, children, ...props }: TooltipProps) {
  return createElement(
    "span",
    { ...props, "data-slot": "tooltip", className: cn("group relative inline-flex", className) },
    children,
    createElement(
      "span",
      {
        ...
        className:
          "pointer-events-none absolute bottom-full left-1/2 z-50 mb-2 hidden -translate-x-1/2 whitespace-nowrap rounded-md bg-foreground px-2 py-1 text-xs text-background shadow-md group-hover:block group-focus-within:block",
      },
      content,
    ),
  );
...
export function MenuSurface({ className, ...props }: HTMLAttributes<HTMLDivElement>) {
  return createElement("div", {
    ...
    className: cn(
      "min-w-40 rounded-md border bg-popover p-1 text-popover-foreground shadow-md",
      className,
    ),
  });
}

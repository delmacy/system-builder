...
function assertSourceProvenance(fields: SupportEvidenceIntakeFields): void {
  ...
  if (fields.sourceKind === "human_request") {
...
export const SupportEvidenceIntake = Object.freeze({
  ...
  validate(value: unknown): SupportEvidenceIntake {
    ...
    for (const key of Object.keys(value)) {
      if (!ALLOWED_FIELDS.has(key)) throw invalid(`UNKNOWN_FIELD:${key}`);
    }
    if (value["kind"] !== "SupportEvidenceIntake") throw invalid("KIND");
    const sourceKind = value["sourceKind"];
    if (sourceKind !== "observe_finding" && sourceKind !== "human_request") {
      throw invalid(`SOURCE_KIND:${String(sourceKind)}`);
    }
    ...
    const fields: SupportEvidenceIntakeFields = Object.freeze({
      ...
      ...(optionalString(value, "publishedReleaseRef") !== undefined
        ? { publishedReleaseRef: optionalString(value, "publishedReleaseRef")! }
        : {}),
    ...
    if (typeof value["intakeId"] !== "string" || value["intakeId"] !== normalized.intakeId) {
      throw invalid("INTAKE_ID");
    }
    return normalized;
  },
});

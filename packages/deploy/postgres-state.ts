...
function postgresConfig(connectionString: string): PostgresConfig {
  ...
  if (sslModeValue !== "disable" && sslModeValue !== "prefer" && sslModeValue !== "require") {
    throw new Error("DEPLOY_POSTGRES_SSLMODE_INVALID");
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    socket.once("connect", () => {
      ...
      socket.once("data", (chunk) => {
        ...
        if (response === 78) {
          if (config.sslMode === "require") finish(undefined, new Error("DEPLOY_POSTGRES_TLS_REQUIRED"));
          else finish(socket);
          return;
        }
        ...
        secure.once("error", () => finish(undefined, new Error("DEPLOY_POSTGRES_TLS_FAILED")));
      });
    });
  });
}
...
async function simpleQuery(connectionString: string, sql: string): Promise<readonly PostgresRow[]> {
  ...
  const socket = await connectPostgres(config);
  return new Promise((resolve, reject) => {
    let settled = false;
    let sent = false;
    ...
    socket.on("data", (chunk) => {
      ...
      while (buffer.length >= 5) {
        ...
        if (type === "R") {
          ...
          if (auth === 5) {
            if (!config.password || payload.length < 8) { finish(new Error("DEPLOY_POSTGRES_PASSWORD_REQUIRED")); return; }
          ...
          if (auth === 11) {
            ...
            if (nonce === undefined || !nonce.startsWith(scram.clientNonce) || salt === undefined || !Number.isInteger(iterations) || iterations <= 0) {
              finish(new Error("DEPLOY_POSTGRES_AUTH_FAILED:SCRAM_CHALLENGE")); return;
            }
          ...
          if (auth === 12) {
            ...
            const finalFields = scramFields(payload.subarray(4).toString("utf8"));
            if (finalFields.get("v") !== scram.serverSignature) { finish(new Error("DEPLOY_POSTGRES_AUTH_FAILED:SCRAM_SIGNATURE")); return; }
            continue;
          }
          finish(new Error(`DEPLOY_POSTGRES_AUTH_UNSUPPORTED:${auth}`)); return;
        } else if (type === "E") {
          const code = errorCode(payload);
          finish(new Error(`${sent ? "DEPLOY_POSTGRES_QUERY_FAILED" : "DEPLOY_POSTGRES_AUTH_FAILED"}:${code}`)); return;

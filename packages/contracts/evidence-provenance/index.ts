...
function normalizeClassification(value: unknown, path: string): EvidenceClassification {
  ...
  if (classification.confidence !== undefined) {
    if (typeof classification.confidence !== "number" || !Number.isFinite(classification.confidence) || classification.confidence < 0 || classification.confidence > 1) {
...
function normalizeTransformation(value: unknown, path: string): EvidenceTransformationDescriptor {
  ...
  const normalized: { descriptorId: string; descriptorVersion: string; tool?: { id: string; version?: string }; provider?: { id: string } } = {
  ...
  if (transformation.tool !== undefined) {
    ...
    const normalizedTool: { id: string; version?: string } = { id: stringAt(tool.id, TOKEN_PATTERN, `${path}.tool.id`) };
...
export function normalizeEvidenceProvenanceExtension(input: unknown): EvidenceProvenanceExtension {
  ...
  const transformations = candidate.transformations.map((transformation, index) => normalizeTransformation(transformation, `$evidenceProvenance.transformations[${index}]`));
